...
import rsl_rl
from rsl_rl.algorithms import PPO, Distillation, SrlPPO
...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu", srl_cfg={}):
        self.cfg = train_cfg
        ...
        # )
        srl_algo_name = srl_cfg["srl_algo_name"]
        print(f"Using SRL algorithm: {srl_algo_name}")
        self.alg_cfg.pop("class_name")
        if srl_algo_name == "ppo":
            self.alg = PPO(policy, device=self.device, **self.alg_cfg, multi_gpu_cfg=self.multi_gpu_cfg)
        else:
            self.alg = SrlPPO(policy, device=self.device, **self.alg_cfg, multi_gpu_cfg=self.multi_gpu_cfg, srl_cfg=srl_cfg)
        ...
        self.alg.init_storage(
            ...
            self.env.pvp_part_size, # NOTE: this is the size of the pvp part of the observation
        )
