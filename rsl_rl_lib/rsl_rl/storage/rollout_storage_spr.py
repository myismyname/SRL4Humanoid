...
class SprRolloutStorage:
    ...
    def mini_batch_generator(self, num_mini_batches, num_epochs=8):
        # NOTE: handle SPR buffer
        spr_observations = torch.clone(self.observations)
        spr_actions = torch.clone(self.actions)
        spr_dones = torch.clone(self.dones)
        # print(spr_observations.size(), spr_actions.size())
        spr_obs_segments = spr_observations.unfold(0, self.spr_k+1, 1)
        spr_actions_segments = spr_actions.unfold(0, self.spr_k+1, 1)
        spr_dones_segments = spr_dones.unfold(0, self.spr_k+1, 1).squeeze(2)
        # print(spr_obs_segments.size(), spr_actions_segments.size(), spr_dones_segments.size())

        has_done = spr_dones_segments.any(dim=2)
        valid_indices = torch.where(torch.logical_not(has_done))
        spr_obs_segments = spr_obs_segments[valid_indices]
        spr_actions_segments = spr_actions_segments[valid_indices]
        spr_mini_batch_size = spr_obs_segments.size()[0] // num_mini_batches
        spr_indices = torch.randperm(spr_mini_batch_size * num_mini_batches, requires_grad=False, device=self.device)
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                # -- For SPR
                spr_start = i * spr_mini_batch_size
                spr_end = (i + 1) * spr_mini_batch_size
                spr_batch_idx = spr_indices[spr_start:spr_end]

                # NOTE: randperm or randint
                # spr_batch_idx = torch.randint(0, spr_obs_segments.size()[0], (mini_batch_size,), device=self.device)
                spr_obs_batch = spr_obs_segments[spr_batch_idx].permute(0, 2, 1)
                spr_actions_batch = spr_actions_segments[spr_batch_idx].permute(0, 2, 1)
                ...
                yield obs_batch, privileged_observations_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, old_mu_batch, old_sigma_batch, (
                    None,
                    None,
                ), None, rnd_state_batch, {'spr_obs_batch': spr_obs_batch, 'spr_actions_batch': spr_actions_batch}
